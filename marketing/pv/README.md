...
2026年9月11日の公開サイトで再収録した最新版。現在の3文字コード・キャラクター・顔5枚の結果表示に対応しています。リアクションはナレーションと動く吹き出しによる演出です。
...
| 18.5–23秒 | 収録で実際に出た3文字コード、キャラクター付き結果カードと、対応する5枚の顔写真 |
| 23–26秒 | 女性・男性それぞれの実際のコードとキャラクター付き結果を並べる |
...
- 動画編集で画面をトリミング、早送り。写真の読み込み中のフレームは直後の実録画フレームまで詰めています。タップ位置の円、テロップ、紙吹雪は編集で追加。結果は実際に表示された一例で、結果カード（キャラクター・結果名・分類・説明）はスクリーンショットを使用。大きなコードは取得した表示値を使用。
...
.venv/bin/python scripts/promo/validate.py
...
`manifest.json` に完成ファイルのサイズ・SHA-256、音声設定、15秒版の編集位置、男女それぞれの結果コード・キャラクター・顔5枚の対応を記録します。動画の再編集後は、完成物と記録をまとめて更新してください。

`validate.py` は完成動画を音声も含めて最後まで復号し、全フレームの寸法・枚数、空の選択カード、音量、MP4の再生開始位置、ファイルハッシュを検査します。収録時の20回答から最多タイプを独立集計し、保存されたコード・キャラクター・5人の顔との一致も確認して `validation.json` を更新します。構図と文字の視認性は確認用画像および完成動画のフレームで別途確認してください。
