...
- The latest 60-person plans use `fringe_down` (自然な下ろし前髪) for everyone. Female outlines are limited to `round`, `short_oval`, and `oval`; female `eyebrow_thickness` must be at most `0.50`. Apply these constraints to baseline identities as well. The v6.1 portraits are stored separately in `assets/planned/v6/<gender>/`, with generated feature records in `data/generated/<gender>_faces_v6.js`; production adoption is a separate step.
