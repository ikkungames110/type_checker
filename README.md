...
- `data/generated/`、`assets/planned/v6/`: ver6.1の設定で生成した男女各60枚の特徴量・画像。診断の出題用データとは別に保存したセット。
...
[新しい120枚の画像一覧](docs/顔画像一覧_ver6.html)で、性別・IDによる絞り込みと顔の拡大表示を利用できます。
内蔵 `image_gen` で全員を個別に生成し、1200×1600に正規化しました。画像は計画の保存先 `assets/planned/v6/<gender>/`、生成済みの特徴量・使用プロンプトは `data/generated/<gender>_faces_v6.js` に保存しています。[制作記録](docs/顔画像生成_ver6.md)に取り込み方法と画像への直接リンクがあります。

トップ画面には、このセットの `female_008` と `male_002` を重ねて表示します。初期表示は女性が手前で、「好みの女性を診断」「好みの男性を診断」のホバー・キーボードフォーカスに合わせて、該当する写真がふわっと手前に移ります。操作対象から外れると女性に戻り、端末の「動きを減らす」設定ではアニメーションを省きます。
...
現行の本番画像は各40枚なので現在は40枚で一巡します。今回生成した60枚セットを本番に採用すると、同じ処理で60枚を使います。[出題処理](js/face_deck.js)は `node --test scripts/test_face_deck.cjs` で検証できます。
