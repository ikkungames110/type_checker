...
最新の画像セットは、全員25歳の設定で男女各8タイプ×5人を個別生成した [80枚の写真比較HTML（ver8）](docs/face-types-v8.html) です。[制作記録](docs/顔タイプ生成_ver8.md)と[所属タイプ・全プロンプト](data/previews/face_types_v8.js)を保存しています。特徴量を持たず、`type` 単位で採点するための比較用画像です。本番診断への採用・採点方式の切り替えはこの画像セットには含みません。以前の [16枚の試作（ver7）](docs/face-types-v7.html) も残しています。
