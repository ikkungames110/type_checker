...
男女各8タイプ・計16枚の新しい試作は [8タイプの写真比較HTML（ver7）](docs/face-types-v7.html) にあります。[制作記録](docs/顔タイプ試作_ver7.md)と[所属タイプ・生成プロンプト](data/previews/face_types_v7.js)を保存しています。ver7は特徴量を持たず、`type` 単位で採点するための画像比較用サンプルです。本番診断への採用・採点方式の切り替えはこの試作には含みません。
