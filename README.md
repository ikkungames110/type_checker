...
- 最多が同点の場合は、該当するタイプをすべて表示します。ASQとRCVが各10票なら `ASQ / RCV` です。抽選はしません。
- 結果には大きなコード、該当キャラ、タイプごとに顔5枚、結果の文字の意味を表示。割合・票数・集計方法の説明・写真の例番号は表示しません。ASQなら「A 軽やか・S 柔らか・Q さりげなさ」を表示します。
